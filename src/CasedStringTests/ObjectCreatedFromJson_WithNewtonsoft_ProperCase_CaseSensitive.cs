...
{
    public class ObjectCreatedFromJson_WithNewtonsoft_ProperCase_CaseSensitive
    {
        public CasedString TestObject { get; set; }

        public ObjectCreatedFromJson_WithNewtonsoft_ProperCase_CaseSensitive()
        {
            string json = $"{{ \"value\": \"{TestStrings.ProperCaseString}\", \"caseSensitive\": true }}";
            CasedString? deserialisedObject = JsonConvert.DeserializeObject<CasedString>(json);
            if (deserialisedObject != null)
            {
                TestObject = deserialisedObject;
            }
            else
            {
